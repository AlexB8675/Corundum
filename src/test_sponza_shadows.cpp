...
int main() {
    ...
    auto shadow_pipeline = crd::make_pipeline(context, renderer, {
        .vertex = "data/shaders/shadow.vert.spv",
        .fragment = "data/shaders/shadow.frag.spv",
    ...
    std::thread([&]() {
        ...
        while (!window.is_closed()) {
            ...
            const auto avg = fps.load() / frames.load();
            std::printf("Average FPS: %lf, Delta Time: %lf\n", avg, delta_time);
            fps = 0;
            frames = 0;
        }
    }).detach();
    while (!window.is_closed()) {
        ...
        main_set[index].bind(context, main_pipeline.bindings["textures"], scene.descriptors);
